...
Generated imports must not be edited to record human decisions. Source-specific review commands copy a selected claim and all of its dependencies into authored `data/research.json`, where it overrides future generated rows with the same ID. For Lobbywatch, use `python review_lobbywatch.py --decision VERIFIED|REJECTED --reviewer ID CLAIM_ID`. A changed source record receives a new versioned claim ID and can therefore be reviewed independently without silently rewriting the prior decision.
